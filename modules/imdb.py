...
def get_mov(url):
    s = get_page_soup(url)
    title = s.find(
        class_="sc-b73cd867-0 eKrKux").text if s.find(class_="sc-b73cd867-0 eKrKux") else ""
    year = s.find(
        class_="sc-8c396aa2-2 itZqyK").text if s.find(class_="sc-8c396aa2-2 itZqyK") else ""
    rating = s.find(
        class_="sc-7ab21ed2-1 jGRxWM").text if s.find(class_="sc-7ab21ed2-1 jGRxWM") else ""
    genres = get_genres(s)
...
def get_movie_info(url):
    soup = get_page_soup(url)
    movie = {"title": soup.find(class_="sc-b73cd867-0 eKrKux").text if soup.find(class_="sc-b73cd867-0 eKrKux") else "", "year": soup.find(class_="sc-8c396aa2-2 itZqyK").text if soup.find(class_="sc-8c396aa2-2 itZqyK") else "", "rating": soup.find(class_="sc-7ab21ed2-1 jGRxWM").text if soup.find(class_="sc-7ab21ed2-1 jGRxWM") else "", "genres": get_genres(
        soup), "videos": get_videos(soup), "images": get_images(soup), "details": get_crew_cast_info(soup)}
    print(movie)
...
def get_crew_cast_info(soup):
        ...
        for actor in soup.find_all(class_="sc-18baf029-7 eVsQmt"):
            ...
            episodes = actor.find(class_="title-cast-item__episodes").text if actor.find(
                class_="title-cast-item__episodes") else None
            cast.append({'name': name, 'profile_url': profile_url,
                        'charector': charector, 'episodes': episodes})
        creators = []
        ...
        return {'cast': cast, 'creators': creators, 'user_review': user_review, 'story': story, 'language': language, 'release_date': release_date, 'aka': aka}
